...
class TestSalaryRule(TestPayslipBase):
    ...
    def test_python_code_return_values(self):
        ...
        line = payslip.line_ids.filtered(lambda line: line.code == "TEST")
    ...
    def test_python_code_result_not_set(self):
        ...
        line = payslip.line_ids.filtered(lambda rule_line: rule_line.code == "TEST")
        self.assertEqual(len(line), 1, "I found the Test line")
    ...
    def test_parent_child_order(self):
        ...
        line = payslip.line_ids.filtered(
            lambda rule_line: rule_line.code == "CHILD_TEST"
        )
        ...
        # Parent and child rule should not be calculated
        # even if child rule condition is true
        parent_line = payslip.line_ids.filtered(
            lambda rule_line: rule_line.code == "PARENT_TEST"
        )
    ...
    def test_rule_and_category_with_and_without_code(self):
        ...
        payslip.compute_sheet()
        line = payslip.line_ids.filtered(
            lambda rule_line: rule_line.code == "rule_test_code"
        )
        self.assertEqual(line.total, 7000, "5000 categories.BASIC + 2000 HRA = 7000")
        line = payslip.line_ids.filtered(
            lambda rule_line: rule_line.name == "rule without code"
        )
        self.assertEqual(len(line), 1, "Line found: rule without code")
        line = payslip.line_ids.filtered(
            lambda rule_line: rule_line.name == "rule without category"
        )
    ...
    def test_line_sum_where(self):
        ...
        payslip.compute_sheet()
        line = payslip.line_ids.filtered(
            lambda rule_line: rule_line.name == "Total fixed values"
        )
