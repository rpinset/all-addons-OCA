...
class TestPDFHelper(TransactionCase):
    def test_get_xml(self):
        pdf_content = read_test_file("pdf_with_xml_test.pdf", mode="rb")
        res = self.env["pdf.xml.tool"].pdf_get_xml_files(pdf_content)
        fname, xml_root = tuple(res.items())[0]
        self.assertEqual(fname, "factur-x.xml")
        self.assertTrue(isinstance(xml_root, etree._Element))
    ...
    def test_get_xml_fail(self):
        with self.assertLogs(
            "odoo.addons.pdf_xml_attachment.models.helper", level="ERROR"
        ) as log_catcher:
            self.env["pdf.xml.tool"].pdf_get_xml_files(b"")
            self.assertIn(
    ...
    def _test_embed_xml(self):
        ...
        newpdf_content = self.env["pdf.xml.tool"].pdf_embed_xml(
            pdf_content, filename, xml
        )
        attachments = self.env["pdf.xml.tool"].pdf_get_xml_files(newpdf_content)
