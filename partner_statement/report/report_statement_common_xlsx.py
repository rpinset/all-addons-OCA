...
class ReportStatementCommonXlsx(models.AbstractModel):
    _name = "report.p_s.report_statement_common_xlsx"
    _description = "Statement Common XLSX Report"
    ...
    def _get_report_name(self, report, data=False):
        ...
        report_name = self._get_report_label()
        if company_id:
    ...
    def _write_currency_title(
        ...
        title_kwargs = self._get_currency_title_kwargs(
            partner_data, account_type, currency
        )
        statement_header = data["get_title"](partner, **title_kwargs)
        sheet.merge_range(
            row_pos, 0, row_pos, 6, statement_header, FORMATS["format_left_bold"]
        )
        return row_pos

    def _iter_currency_transaction_lines(
    ...
    def _write_currency_lines(self, row_pos, sheet, partner, currency, data):
        ...
        row_pos += 2
        row_pos = self._write_currency_title(
        ...
        row_pos = self._iter_currency_transaction_lines(
    ...
    def _write_currency_buckets(self, row_pos, sheet, partner, currency, data):
        report_model = self.env[self._get_report_model_name()]
        partner_data = data.get("data", {}).get(partner.id, {})
        currency_data = partner_data.get("currencies", {}).get(currency.id)
    ...
    def _write_workbook_title(self, sheet, row_pos, data, company):
        sheet.merge_range(
            row_pos,
            0,
            row_pos,
    ...
    def _write_workbook_date(self, sheet, row_pos, data, partners):
        row_pos += 1
        sheet.write(
    ...
    def _write_pre_partners(self, sheet, row_pos, data, partners, company):
        return row_pos

    def _write_post_partners(self, sheet, row_pos, data, partners, company):
        return row_pos

    def _write_partner_address_block(
        self, sheet, row_pos, data, partner, company, invoice_address
    ):
        row_pos += 3
        sheet.write(
            row_pos,
    ...
    def _setup_currency_money_format(self, workbook, currency):
        if currency.position == "after":
            money_string = (
                "#,##0.%s " % ("0" * currency.decimal_places) + f"[${currency.symbol}]"
            )
    ...
    def _write_partner(self, workbook, sheet, row_pos, data, partner, company):
        invoice_address = data.get("get_inv_addr", lambda x: self.env["res.partner"])(
            partner
        )
        row_pos = self._write_partner_address_block(
            sheet, row_pos, data, partner, company, invoice_address
        )
        row_pos = self._iter_partner_currencies(workbook, sheet, row_pos, data, partner)
        return row_pos
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        sheet = workbook.add_worksheet(self._get_report_label())
        sheet.set_landscape()
        row_pos = 0
