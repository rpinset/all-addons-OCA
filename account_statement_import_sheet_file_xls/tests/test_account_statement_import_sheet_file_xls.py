...
class TestAccountStatementImportSheetFileXls(
    test_account_statement_import_sheet_file.TestAccountStatementImportSheetFile
):
    def _get_file_path(self, filename):
        # It needs to get correct file path
        return path.join(path.dirname(__file__), filename)

    def test_import_xls_file(self):
        wizard = self._get_import_wizard("fixtures/sample_statement_en.xls")
    ...
    def test_metadata_separated_debit_credit_xls(self):
        ...
        wizard = self._get_import_wizard(
            "fixtures/meta_data_separated_credit_debit.xls"
        )
        wizard.import_file_button()
        statement = self.AccountBankStatement.search(self.statement_domain)
        self.assertEqual(len(statement), 1)
    ...
    def test_import_xls_empty_values(self):
        ...
        wizard = self._get_import_wizard(
            "fixtures/sample_statement_en_empty_values.xls"
        )
    ...
    def test_offsets(self):
        ...
        file_name = "fixtures/sample_statement_offsets.xls"
