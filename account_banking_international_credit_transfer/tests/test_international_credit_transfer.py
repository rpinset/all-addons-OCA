...
class TestSCT(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        (cls.partner_c2c + cls.partner_agrolait).with_company(
            cls.main_company.id
                    ...
                    (
                        ...
                        0,
                        {
                            "payment_method_id": cls.env.ref(
                                "account_banking_international_credit_transfer.international_credit_transfer"
                            ).id,
    ...
    def test_no_pain(self):
        ...
        with self.assertRaises(UserError):
            self.check_international_credit_transfer()

    def test_no_intermediary_bank(self):
        self.payment_mode.payment_method_id.pain_version = "pain.001.001.03"
        self.partner_bank_1.intermediary_bank_id = False
        self.partner_bank_2.intermediary_bank_id = False
        with self.assertRaises(UserError):
            self.check_international_credit_transfer()

    def test_pain_001_03(self):
        self.payment_mode.payment_method_id.pain_version = "pain.001.001.03"
        self.partner_bank_1.intermediary_bank_id = self.bank_2.id
        self.partner_bank_2.intermediary_bank_id = self.bank_1.id
        self.check_international_credit_transfer()

    def check_international_credit_transfer(self):
        ...
        intermediary_bank_xpath = xml_root.xpath(
            "//p:PmtInf/p:CdtTrfTxInf/p:IntrmyAgt1/p:FinInstnId",
            namespaces=namespaces,
        )
        intermediary_bic = intermediary_bank_xpath[0].xpath(
            "./p:BIC", namespaces=namespaces
        )
        intermediary_name = intermediary_bank_xpath[0].xpath(
            "./p:Nm", namespaces=namespaces
        )
        self.assertEqual(
            intermediary_bic[0].text, self.partner_bank_1.intermediary_bank_id.bic
        )
        self.assertEqual(
            intermediary_name[0].text, self.partner_bank_1.intermediary_bank_id.name
        )
