...
class TierValidation(models.AbstractModel):
    ...
    def view_tier_correction(self):
        self.ensure_one()
        # Caller document
        res_model = self.env["ir.model"].search(Domain("model", "=", self._name))
        ...
        items = self.env["tier.correction.item"].search_fetch(
            Domain("res_model", "=", self._name) & Domain("res_id", "=", self.id),
            ["correction_id"],
        )
        if len(items) > 0:  # Already has corrections, show them.
            result["domain"] = list(
                Domain("id", "in", items.mapped("correction_id").ids)
            )
