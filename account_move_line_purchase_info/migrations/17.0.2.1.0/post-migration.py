...
def migrate(cr, version):
    ...
    for move in moves:
        broken = move.line_ids.filtered(
            lambda line: line.display_type == "cogs" and not line.oca_purchase_line_id
        )
        # Respect any POL already linked to other COGS
        # lines in the move
        consumed = set(
            move.line_ids.filtered(
                lambda line: line.display_type == "cogs" and line.oca_purchase_line_id
            )
        ...
        for (product_id, quantity), group in groups_by_pq.items():
            ...
            candidates = move.invoice_line_ids.filtered(
                lambda il, product_id=product_id, quantity=quantity: il.product_id.id
                == product_id
            ...
            if len(accounts) != 2:
                ...
                continue
            rows_a = group.filtered(
                lambda line, accounts=accounts: line.account_id == accounts[0]
            ).sorted("id")
            rows_b = group.filtered(
                lambda line, accounts=accounts: line.account_id == accounts[1]
            ).sorted("id")
